...
if [ "$TOKEN" != "null" ]; then
    ...
    if [ "$RELEASE" == "TEST" ]; then
        ...
        docker manifest create ${DOCKER_IMAGE}:${ALPINE_VERSION}-${MAXMIND_GEOLITE_VERSION} \
            --amend ${DOCKER_IMAGE}:${ALPINE_VERSION}-${MAXMIND_GEOLITE_VERSION}-aarch64 \
        ...
        docker manifest create ${DOCKER_IMAGE}:${MAXMIND_GEOLITE_VERSION} \
            --amend ${DOCKER_IMAGE}:${MAXMIND_GEOLITE_VERSION}-aarch64 \
