...
IMAGE=maurosoft1973/alpine-maxmind-geolite:test
CONTAINER=alpine-maxmind-geolite-test
LC_ALL=it_IT.UTF-8
...
docker run -dit --name ${CONTAINER} -p ${IP}:${PORT}:${PORT} -e LC_ALL=${LC_ALL} -e PORT=${PORT} ${IMAGE}
